...
class ProjectActivityLogAdmin(admin.ModelAdmin):
    ...
    # Робить поля тільки для читання, щоб адмін не підробив історію
    readonly_fields = ('project', 'actor', 'action_type', 'target', 'timestamp')

    # Забороняє додавати логи вручну (вони мають писатися тільки автоматично через сигнали)
    def has_add_permission(self, request):
        return False

    # Забороняє видаляти логи (опціонально)
